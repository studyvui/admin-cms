// ============================================================
// Sinh cau hoi that Toan Lop 3 tu GRADE3_BUILTIN_TEMPLATES (Phase 3, 2026-08-05).
...
// cho script Python ghi len production (POST /admin/questions/bulk-upload).
//
// Chay: npx tsx scripts/gen-grade3-questions.ts > /path/to/phase3_g3_rows.json
// ============================================================
import { GRADE3_BUILTIN_TEMPLATES } from "../lib/math-gen/builtins-grade3";
...
function main() {
  ...
  for (let week = 1; week <= 35; week++) {
    ...
    const templates = GRADE3_BUILTIN_TEMPLATES.filter((t) => t.id.startsWith(`TPL_G3_W${wk}_`));
    ...
    while (rows.length < TARGET_PER_WEEK && guard < TARGET_PER_WEEK * MAX_ATTEMPTS_PER_Q) {
      ...
      // (mot so archetype pham vi output rat hep, vd "hinh tam giac co may canh?").
